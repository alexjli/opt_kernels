...
def generate_fwd_kernel(channel_dim, bias_dim, dtype):
    ...
    def ipa_sdpa_fwd(
        ...
        # v_pts: dim5_tensor_type,    # [B, H, V_pts, KV, 3]
        ...
        # out_pts: dim5_tensor_type,  # [B, H, V_pts, Q, 3]
        ...
        # block_q_pts = q_pts[batch, head]
        # block_k_pts = k_pts[batch, head]
        # block_v_pts = v_pts[batch, head]
        block_out_bias = z_out_bias[batch]
        block_attn_bias = z_attn_bias[batch, head]
        block_L = L[batch, head]
        block_M = M[batch, head]
        block_out = out[batch, head]

        out_bias_tile = wp.tile_zeros(BLOCK_SIZE, bias_dim, dtype=float)
        ...
        for block_j in range(num_k_blocks):
            ...
            # pt_dist_buffer = wp.tile_zeros(BLOCK_SIZE, BLOCK_SIZE, dtype=float)

            # q_pts_tile = wp.tile_load(block_q_pts, block_i, 0, BLOCK_SIZE, n_qk_pts)
            # k_pts_tile = wp.tile_load(block_k_pts, block_j, 0, BLOCK_SIZE, n_qk_pts)

            # for p in range(n_qk_pts):
            #     pt_dist_buffer *= 0.
            #     pt_q = wp.tile_broadcast(
            #         wp.tile_transpose(q_pts_tile)[p],
            #         m=BLOCK_SIZE,
            #         n=BLOCK_SIZE
            #     )
            #     pt_k = wp.tile_broadcast(
            #         wp.tile_transpose(wp.tile_transpose(k_pts_tile)[p]),
            #         m=BLOCK_SIZE,
            #         n=BLOCK_SIZE
            #     )
            #     pt_disp = wp.tile_map(wp.sub, pt_q, pt_k)

            #     pt_dist_update = wp.tile_map(wp.length_sq, pt_disp)
            #     wp.tile_assign(pt_dist_buffer, 0, 0, pt_dist_update)
            #     pt_dist += pt_dist_buffer
            ...
            # for pt_v in range(n_v_pts):
            #     out_pts_tile = wp.tile_load(out_pts[batch, head, pt_v], block_i, 0, BLOCK_SIZE, 3)
            #     out_pts_tile = (
            #         wp.tile_matmul(old_scale_diag, out_pts_tile) +
            #         wp.tile_matmul(
            #             new_scale,
            #             wp.tile_load(block_v_pts[pt_v], block_j, 0, BLOCK_SIZE, 3)
            #         )
            #     )
            #     out_pts_tile = wp.tile_map(wp.div, out_pts_tile, pt_div_scale)
            #     wp.tile_store(out_pts[batch, head, pt_v], block_i, 0, out_pts_tile)

            # for i in range(len_i):
            #     block_out_bias = z_out_bias[batch, block_i * BLOCK_SIZE + i]
            #     update = wp.tile_load(block_out_bias, block_j, 0, BLOCK_SIZE, bias_dim)
            #     update = wp.tile_matmul(
            #         new_scale[i],
            #         update
            #     )
            #     out_bias_tile_row = out_bias_tile[i]
            #     new_row = (
            #         wp.tile_matmul(old_scale[i], out_bias_tile_row)
            #         + update
            #     )
            #     new_row = wp.tile_map(wp.div, new_row, bias_div_scale[i])
            #     wp.tile_assign(out_bias_tile_row, 0, 0, new_row)

            wp.tile_store(block_L, block_i, l_new)
            wp.tile_store(block_M, block_i, m_new)
...
if __name__ == '__main__':
    ...
    v_pts = torch.randn((B, H, v_pts, L, 3), dtype=torch.float32, device=device)
